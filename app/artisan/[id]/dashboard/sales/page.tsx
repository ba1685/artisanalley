...
export default async function SalesAndOrdersPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 1. Fetch all orders where the artwork belongs to THIS artisan
  const orders = await prisma.order.findMany({
    ...
    include: {
      artwork: true,    // To show what they bought
        ...
        {orders.length === 0 ? (
          ...
          orders.map((order) => (
            <div key={order.id} className="bg-[#FAF9F6] border border-[#E5E1DA] rounded-2xl p-8 shadow-sm flex flex-col md:flex-row gap-8">
              ...
              {/* Artwork Thumbnail */}
              <div className="w-full md:w-48 aspect-square bg-[#F2EFE9] rounded-lg overflow-hidden flex-shrink-0 border border-[#E5E1DA]">
                {order.artwork.imageUrl ? (
                  <img src={order.artwork.imageUrl} alt={order.artwork.title} className="w-full h-full object-cover mix-multiply opacity-90" />
                ) : (
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <h3 className="font-serif text-2xl text-[#2C2926]">{order.artwork.title}</h3>
                    </div>
                    <div className="text-right">
                      <p className="font-serif text-2xl text-[#2C2926]">₹{order.amount.toLocaleString()}</p>
