...
// 1. REMOVED { params } completely since this is a static route
export default async function ArtisanDashboard() {
  
  // 2. Hardcoded email for testing. (Replace with real auth session later)
  const loggedInArtisanEmail = "test@artisanalley.com";

  // 3. Search Prisma using the email
  ...
  // 4. Safety Check: Prevents a crash if the test email isn't in your DB yet
  if (!artisan) {
    return (
  ...
  return (
    <div className="flex min-h-screen bg-[#F9F7F2] font-sans text-[#4A443F]">
      ...
      {/* Sidebar - Matching your aesthetic */}
      <aside className="w-64 border-r border-[#E5E1DA] p-8 flex flex-col justify-between">
        <div>
          ...
          <nav className="space-y-6 text-[11px] uppercase tracking-widest font-semibold text-[#8C847C]">
            ...
            {/* Updated Links to match the static routing structure */}
            <Link href="/artisan/upload" className="block hover:text-[#2C2926] transition-colors">Upload Artwork</Link>
            <Link href="/artisan/settings" className="block hover:text-[#2C2926] transition-colors">Settings</Link>
          </nav>
        </div>
      </aside>
